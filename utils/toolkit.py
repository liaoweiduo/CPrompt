...
def accuracy(y_pred, y_true, nb_old, increment=10, loss=False):
    ...
    for class_id in range(0, np.max(y_true), increment):
        ...
        if loss:
            all_acc[label] = np.around(np.mean(y_pred[idxes]), decimals=4)
        else:
    ...
    if loss:
        all_acc['old'] = np.around(np.mean(y_pred[idxes]), decimals=4)
    else:
    ...
    idxes = np.where(y_true >= nb_old)[0]
    if loss:
        all_acc['new'] = np.around(np.mean(y_pred[idxes]), decimals=4)
    else:
